...
<div class="container mx-auto p-6">
    <h1 class="text-3xl font-bold mb-6">
        ...
        @if(isset($selectedCategory))
            - {{ $selectedCategory->name }} ({{ $products->total() }} items)
        @endif
        @if(isset($selectedBrand))
            - {{ $selectedBrand->name }} ({{ $products->total() }} items)
        @endif
    </h1>
            ...
            <div id="product-list" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($products as $product)
                    <a href="{{ route('product.show', ['id' => $product->id]) }}" 
                       class="group relative bg-white rounded-2xl border border-gray-100 p-4 transition-all hover:shadow-lg">
